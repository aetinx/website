  ...
  useEffect(() => {
    ...
    async function setup() {
      defaultTapSoundBuffer.current = await getSoundBuffer(
        sounds["/src/assets/sfx/win7/Windows Navigation Start.wav"]
        //sounds["/src/assets/sfx/honk/Navigation & Toggles/ui general click.wav"]
      )
      /*carouselTapSoundBuffer.current = await getSoundBuffer(
        sounds["/src/assets/sfx/honk/Navigation & Toggles/toggle minor b.wav"]
      )*/
    }
    setup()
    ...
    async function onClick(event) {
      ...
      //if (event.pointerType !== "mouse" && !soundPath) return
